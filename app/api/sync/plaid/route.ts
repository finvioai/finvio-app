...
export async function POST() {
  ...
  if (!connection) {
    return NextResponse.json({ error: 'No active Plaid connection. Connect a bank first.' }, { status: 400 })
  }

  const result = await syncPlaidTransactions(member.org_id, connection.id, supabase)
