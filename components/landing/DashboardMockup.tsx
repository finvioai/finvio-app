      ...
      <div className="flex min-h-[520px] gap-4 overflow-hidden rounded-xl bg-off-white p-4">
        ...
        <aside className="hidden w-48 shrink-0 flex-col gap-2 rounded-lg bg-background p-3 ring-1 ring-hairline md:flex">
          ...
          <div className="mt-2 space-y-1">
            {['Overview', 'Treasury', 'Reconciliation', 'Tax & Forms', 'Invoices', 'Insights'].map(
              (label, i) => (
                <div
                  key={label}
...
function KpiCard({
  ...
  return (
    <div className="rounded-xl bg-background p-4 ring-1 ring-hairline">
      ...
      <div className={`mt-2 text-2xl font-bold ${accent ? 'text-brand' : 'text-navy'}`}>{value}</div>
      <div className={`mt-2 text-[10px] font-semibold ${positive ? 'text-emerald-600' : 'text-muted-ink'}`}>
        {delta}
      </div>
    </div>
  )
