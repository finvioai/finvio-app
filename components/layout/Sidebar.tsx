...
export function Sidebar({ userEmail, orgName }: SidebarProps) {
  ...
  return (
    <aside className="flex h-screen w-60 flex-col bg-navy">
      {/* Logo */}
      <div className="flex h-16 items-center gap-2.5 px-5 border-b border-white/8">
      ...
      <div className="border-t border-white/8 p-3">
        <DropdownMenu>
          <DropdownMenuTrigger className="flex w-full items-center gap-3 rounded-lg px-2 py-2 text-sm hover:bg-white/6 transition-colors outline-none">
            <Avatar className="h-8 w-8 shrink-0">
              <AvatarFallback className="bg-brand/25 text-brand text-xs font-semibold">
                {initials}
              </AvatarFallback>
            </Avatar>
            <div className="flex-1 text-left overflow-hidden">
              <p className="text-xs font-medium text-navy-foreground truncate">{userEmail}</p>
              <p className="text-xs text-navy-foreground/50">Free plan</p>
            </div>
            <ChevronDown className="h-3.5 w-3.5 text-navy-foreground/40 shrink-0" />
          </DropdownMenuTrigger>
