...
| Voice Input | Press-and-hold mic on mobile (walkie-talkie style, auto-sends on release); click-to-toggle on desktop with live word-by-word transcript overlay; Web Speech API is free with no server round-trip; server Whisper (`whisper-1`) used on Brave/Firefox at ~$0.006/min, gated by per-user daily (5 min) and monthly (1 hr) quotas tracked in `voice_usage` table; over-quota users transparently routed to in-browser WASM (Xenova/whisper-tiny via `@huggingface/transformers`); low-end devices (<4 CPU cores or <4 GB RAM) that exceed quota get a hard block with no silent server escape; Brave detected via synchronous `navigator.brave` check |
